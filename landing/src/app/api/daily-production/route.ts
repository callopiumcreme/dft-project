...
// Mirrors backend.app.routers.daily_production.list_production() query
// params. limit + offset support the client-side "Show more" pagination.
const ALLOWED_QS = new Set(['date_from', 'date_to', 'limit', 'offset']);
...
export async function GET(req: NextRequest) {
  ...
  try {
    const upstream = await apiFetch(`/daily-production${suffix}`, {
