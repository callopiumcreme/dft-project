...
3. Followed by interim written status report on 30 May 2026 against the milestone plan.
4. Final submission on ROS by 13 June 2026.

**Status:** Draft pending Crown Oil signatory + nominated DfT contact.

**Attachments to include:**
- Curated screenshots from the OisteBio production / mass-balance management platform (daily view, monthly aggregate, supplier breakdown, audit log, KPI dashboard, evidence register, cryptographic export sample)
- Annex E — Milestone plan (tabular, 13 May → 13 June 2026)

Full evidence package (Annexes A–D, supplier remediation, retroactive PoS ISCC, etc.) is committed as the deliverable at the end of the 30-day window, not as a precondition to opening this request.
...
- audit-grade litres conversion via a time-effective production density schema (EU and Plus densities sourced from EAD documentation), reconcilable from kg to litres for every production day;
- collecting-point and supplier classification schema sufficient to map every kilogram of input to its ISCC-certified collecting point;
- cryptographic export of mass-balance evidence (PDF with embedded SHA-256 hash) for tamper-evident delivery;
- KPI dashboards, drill-down reports, CSV export, and role-based access control (admin, operator, viewer, certifier) sufficient for both day-to-day operation and verification access.

This is not the same artefact as the spreadsheets and PDF folders the Unit has previously received. It is a different kind of deliverable, and it is the foundation on which we propose to assemble the 2025 submission.

**3. What is enclosed with this request**

So that the Unit can see what we are referring to without taking our word for it, we enclose **curated sample screenshots** from the platform — daily mass-balance view, monthly aggregate, supplier breakdown, audit log entry, KPI dashboard, cryptographic export sample — together with **Annex E (milestone plan)** in tabular form covering the 13 May → 13 June window.

The screenshots are illustrative, not exhaustive. They are intended to allow the Unit to form a view of the deliverable's nature at this stage; the deliverable itself is what we propose to submit at the end of the window.

We have deliberately not attached an early body of evidence in the format of prior submissions. The previous format did not serve the Unit's purposes, and we would rather not add to that file. The submission on 13 June 2026 is intended to replace it, not to extend it.

**4. How the 30 days will be used**
...
- the final submission body is assembled, internally audited end-to-end (every kilogram cross-referenced to its PoS ISCC and provider registration), and exported with cryptographic integrity verification.

The interim status report on 30 May 2026 will give the Unit a checkpoint at the midpoint against each of these workstreams.

**5. Why we are asking, and what we are asking for**
...
The primary point of contact for this request is [Crown Oil compliance lead, email, phone]. OisteBio (cc'd) is available to respond directly on technical questions concerning the platform, the mass-balance engine, the audit log, or any aspect of the digital infrastructure. We can also arrange, at the Unit's convenience, a brief live demonstration of the platform during the 30-day window — for example at the 30 May 2026 checkpoint — should that be useful.

We remain at the Unit's disposal for a further call. We will revert with the interim status report on 30 May 2026 in any event, and would welcome confirmation that the extension to 13 June 2026 is granted at the Unit's earliest convenience.
